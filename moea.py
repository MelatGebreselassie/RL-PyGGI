...
from greedy import MyTreeProgram

class MoeaNsgaProgram(MyTreeProgram):
    def setup(self, config):
        super().setup(config)
        self.target_files = ['DMOEA/dmoeafunc.h.xml', 'common/recombination.h.xml']

        self.instances = [str(k) for k in range(0, 9)]
        self.seeds = list(range(1, 3)) # 1, 2, ... 10
        # self.instances = ['0']
        # self.seeds = list(range(1, 2)) # 8
